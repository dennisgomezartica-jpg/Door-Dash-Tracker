...
export default function App() {
  ...
  const blankForm = { date:new Date().toISOString().slice(0,10), hours:"", miles:"", gross:"", timeWindow:"5PM–9PM", day:DAYS[new Date().getDay()===0?6:new Date().getDay()-1], rained:false, gas:"", carMaint:"", notes:"", city:"Newnan GA" };
  ...
  function handleFormChange(e) { const {name,value,type,checked}=e.target; setForm(f=>({...f,[name]:type==="checkbox"?checked:value})); }
          ...
          {!eventsLoading && events.length > 0 && (
            <>
              ...
              {events.map((e,i) => {
                ...
                return (
                  <div key={e.id||i} style={{ padding:"12px 14px", background:C.surface2, borderRadius:12, marginBottom:8, border:`0.5px solid ${C.border}` }}>
                    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
                      <div style={{ display:"flex", gap:10, alignItems:"flex-start" }}>
                        ...
                        <div>
                          <div style={{ display:"flex", alignItems:"center", gap:6, marginBottom:3 }}>
                            ...
                            <span style={{ fontFamily:"'DM Sans',sans-serif", fontSize:11, fontWeight:600, color:C.muted, textTransform:"uppercase", letterSpacing:"0.05em" }}>{e.sport}</span>
                          </div>
                      ...
                      <div style={{ textAlign:"right", flexShrink:0 }}>
                        ...
                        <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:10, color:C.muted, marginTop:2 }}>{e.isHome ? "Home" : "Away"}</div>
                      </div>
                    </div>
                  </div>
                );
